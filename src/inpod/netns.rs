...
impl InpodNetns {
    ...
    where
        F: FnOnce() -> T,
    {
        tracing::debug!(target: "ztunnel_netns", workload_netns_id = ?self.inner.netns_id, "switching to workload netns");
        ...
        match nix::sys::stat::fstat(self.inner.cur_netns.as_raw_fd()) {
            Ok(stat) => {
                tracing::debug!(target: "ztunnel_netns", original_netns_id = ?NetnsID{inode: stat.st_ino, dev: stat.st_dev}, "switched back to original netns");
            }
            Err(e) => {
                tracing::debug!(target: "ztunnel_netns", error = ?e, "switched back to original netns, but failed to get original netns_id");
            }
        }
        Ok(ret)
    }
