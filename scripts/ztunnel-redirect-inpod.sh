...
echo "[ZTUNNEL_IPTABLES] Attempting to restore iptables rules..."
...
COMMIT
echo "[ZTUNNEL_IPTABLES] Successfully applied *mangle table rules."
*nat
:PREROUTING ACCEPT [0:0]
:INPUT ACCEPT [0:0]
:OUTPUT ACCEPT [0:0]
...
COMMIT
echo "[ZTUNNEL_IPTABLES] Successfully applied *nat table rules."
EOF

echo "[ZTUNNEL_IPTABLES] Attempting to add local route for table 100..."
ip route add local 0.0.0.0/0 dev lo table 100 || :

# tproxy and original src
echo "[ZTUNNEL_IPTABLES] Attempting to add firewall rule for mark 0x111/0xfff..."
